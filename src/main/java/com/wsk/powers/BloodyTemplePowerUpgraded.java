...
public class BloodyTemplePowerUpgraded extends AbstractPower {
    public static final String POWER_ID = "MyMod:BloodyTemplePowerUpgraded";//能力的ID，判断有无能力、能力层数时填写该Id而不是类名。
    ...
    public BloodyTemplePowerUpgraded(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
    ...
    public void updateDescription() {
        this.description = (DESCRIPTIONS[0] + (this.amount * 2));
    }
