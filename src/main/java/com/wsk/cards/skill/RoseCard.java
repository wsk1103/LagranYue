...
public class RoseCard extends CustomCard {
    ...
    //增加的力量
    private static final int addStrength = 30;
...
//            this.isInnate = false;//固有属性。
            //update card and change description
    ...
    public void use(AbstractPlayer p, AbstractMonster abstractMonster) {
        AbstractDungeon.actionManager.addToBottom(new ApplyPowerAction(p, p, new StrengthPower(p, addStrength), addStrength));
        if (upgraded) {
            //人工制品
            AbstractDungeon.actionManager.addToBottom(new ApplyPowerAction(p, p, new ArtifactPower(p, addStrength), addStrength));
        }
